...
const Index = (props) => {
  ...
  const calledit = () => {
    const config = {
      method: 'get',
      url: `${process.env.APP_API}statistik/edit` + props.id,
      headers: {
  ...
  const onSubmit = data => {
    const config = {
      method: 'post',
      url: `${process.env.APP_API}statistik/update` + props.id,
      headers: {
        Authorization: `Bearer ${localStorage.getItem('accessToken')}`
      },
      data: data
  ...
  return (
    <>
      <Card>
        <CardContent>
          <Header>
            <Typography variant='h5'>{`Edit Statistik `}</Typography>
            <IconButton
              size='small'
              onClick={handleClose}
              sx={{
