...
const List = () => {
  ...
  const fetchTableData = useCallback(
    async (sort, q, column,
      ...
      jenjang,) => {
      await axios
        .get(`${process.env.APP_API}master/distributor`, {
          headers: {
  ...
  useEffect(() => {
    const calltahun = async () => {
      await axios.get(`${process.env.APP_API}master/distributor`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('accessToken')}`
        },
      }).then((res) => {
          ...
          >
            ...
            <Grid item xs={12} sm={6}>
              <Button
                ...
                sx={{ '& svg': { mr: 2 } }}
                onClick={() => {
                  const url = `/report/all/distributor`
