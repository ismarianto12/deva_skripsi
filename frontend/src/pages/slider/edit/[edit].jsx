...
const Index = props => {
  ...
  useEffect(() => {
    ...
    Action().CallEditSlider({ setEditData, reset, editid })
    console.log(editdata, 'slider edit')
    call()
    calltag()
  }, []);
  const {
  ...
  const onSubmit = async (data) => {
    if (data.gambar === '') {
      ...
      try {
        ...
        formData.append('describeIn', data.describeIn);
        formData.append('describeEn', data.describeEn);
  ...
  return (
    <>
      <Headtitle title="Edit Data Slider" />
      <Card>
        <CardContent>
          <Header>
            <Typography variant='h5'>
              <Icon icon='tabler:create' fontSize={20} />
              {`Edit Slider`}</Typography>
            <IconButton
              size='small'
              onClick={handleClose}
              sx={{
