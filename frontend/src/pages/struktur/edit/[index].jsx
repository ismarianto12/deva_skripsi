...
const Index = props => {
  ...
  const callEdit = () => {
    setLoading(true)
    axios.get(`${process.env.APP_API}struktur/detail/${props.id}`, {
      headers: {
      ...
      setLoading(false)
      reset(res.data)
      setContenin(res.data.riwayat_en)
      setContenEn(res.data.riwayat_idn)
      setReqdata(res.data)
  ...
  console.log(reqdata.bagian, 'test bagian')
  const {
    reset,
    control,
    setValue,
    ...
    defaultValues: {
      ...
      bagian: "",  //reqdata.bagian ? reqdata.bagian : 'Komisaris',
      riwayat_id: "", //reqdata.riwayat_id,
      riwayat_en: "" // reqdata.riwayat_en
    },
    mode: 'onChange',
  ...
  const onSubmit = async (data) => {
    if (data.picture[0] === '') {
      ...
      try {
        ...
        await axios.post(`${process.env.APP_API}struktur/update/${props.id}`, formData, {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('accessToken')}`,
          },
        }).then(() => {
  ...
  return (
    <>
      <Headtitle title="Edit Struktur Korporasi" />
        ...
        <CardContent>
          <Header>
            <Typography variant='h5'>
              ...
              {`Edit Struktur Korporasi`}</Typography>
            <IconButton
              size='small'
              onClick={handleClose}
              sx={{
          ...
          <Box sx={{ p: theme => theme.spacing(0, 6, 6) }}>
            <form onSubmit={handleSubmit(onSubmit)}>
                  ...
                  <Controller
                    ...
                    render={({ field: { value, onChange } }) => (
                      <CustomTextField
                        ...
                        placeholder='jabatan'
                        {...(errors.jabatan && { helperText: errors.jabatan.message })}
                      />
                    )}
                  />
                </Grid>
              ...
              <br /><br />
              {reqdata.picture && (<img src={`${process.env.ASSETS_API}/struktur/${reqdata.picture}`} style={{ width: '40%' }} />)}
